...
class DabDetrMultiheadAttention(nn.Module):
    ...
    Args:
        embed_dims (int): The embedding dimension for attention.
        ...
        batch_first (bool): if `True`, then the input and output tensor will be
            provided as `(bs, n, embed_dims)`. Default: False. `(n, bs, embed_dims)`
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DabDetrMultiheadAttention, self).__init__()
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        Args:
            query (torch.Tensor): Query embeddings with shape
                `(num_query, bs, embed_dims)` if self.batch_first is False,
                else `(bs, num_query, embed_dims)`
            key (torch.Tensor): Key embeddings with shape
                `(num_key, bs, embed_dims)` if self.batch_first is False,
                else `(bs, num_key, embed_dims)`
...
class ConditionalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_content_proj = nn.Linear(embed_dims, embed_dims)
        self.query_pos_proj = nn.Linear(embed_dims, embed_dims)
        self.key_content_proj = nn.Linear(embed_dims, embed_dims)
        self.key_pos_proj = nn.Linear(embed_dims, embed_dims)
...
class ConditionalCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.query_content_proj = nn.Linear(embed_dims, embed_dims)
        self.query_pos_proj = nn.Linear(embed_dims, embed_dims)
        self.query_pos_sine_proj = nn.Linear(embed_dims, embed_dims)
        self.key_content_proj = nn.Linear(embed_dims, embed_dims)
        self.key_pos_proj = nn.Linear(embed_dims, embed_dims)
        self.value_proj = nn.Linear(embed_dims, embed_dims)
        self.out_proj = nn.Linear(embed_dims, embed_dims)
